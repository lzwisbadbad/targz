...
// ABCIQueryWithOptions exposes all options for the ABCI query and verifies the returned proof
func (w Wrapper) ABCIQueryExWithOptions(path string) (*ctypes.ResultABCIQueryEx, error) {

	res, _, err := GetWithProofOptionsEx(path, w.Client, w.cert)
	return res, err
}

// ABCIQuery uses default options for the ABCI query and verifies the returned proof
func (w Wrapper) ABCIQueryEx(path string) (*ctypes.ResultABCIQueryEx, error) {
	return w.ABCIQueryExWithOptions(path)
}
