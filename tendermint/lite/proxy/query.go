...
import (
	...
	"github.com/bcbchain/tendermint/lite/client"
	certerr "github.com/bcbchain/tendermint/lite/errors"
	rpcclient "github.com/bcbchain/tendermint/rpc/client"
...
func GetWithProofOptions(path string, key []byte, opts rpcclient.ABCIQueryOptions,
	...
	*ctypes.ResultABCIQuery, KeyProof, error) {
	...
	/* // TODO refactor so iavl stuff is not in tendermint core
	   // https://github.com/bcbchain/tendermint/issues/1183
	if len(resp.Value) > 0 {
...
// GetWithProofOptions is useful if you want full access to the ABCIQueryOptions
func GetWithProofOptionsEx(path string,
	node rpcclient.Client, cert lite.Certifier) (
	*ctypes.ResultABCIQueryEx, KeyProof, error) {

	_resp, err := node.ABCIQueryExWithOptions(path)
	if err != nil {
		return nil, nil, err
	}
	resp := _resp.Response
	...
	// make sure the proof is the proper height
	if resp.IsErr() {
		err = errors.Errorf("Query error for path %d: %d", path, resp.Code)
		return nil, nil, err
	}
	if len(resp.KeyValues) == 0 || len(resp.Proof) == 0 {
		return nil, nil, ErrNoData()
	}
	if resp.Height == 0 {
		return nil, nil, errors.New("Height returned is zero")
	...
	_ = commit
	return &ctypes.ResultABCIQueryEx{Response: resp}, nil, nil
