...
func BenchmarkValidatorSetCopy(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		...
		val := NewValidator(pubKey, 0, "reward", "name")
		if !vset.Add(val) {
...
func TestProposerSelection1(t *testing.T) {
	vset := NewValidatorSet([]*Validator{
		newValidator("foo", 1000),
		newValidator("bar", 300),
		newValidator("baz", 330),
	})
	var proposers []string
	for i := 0; i < 99; i++ {
		val := vset.GetProposer(nil, nil)
...
func TestProposerSelection3(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		proposerOrder[i] = vset.GetProposer(nil, nil)
		vset.IncrementAccum(1)
	}
	...
	var i, j int
	for ; i < 10000; i++ {
		got := vset.GetProposer(nil, nil).Address
		...
		computed := vset.GetProposer(nil, nil) // findGetProposer()
		if i != 0 {
...
func newValidator(address string, power uint64) *Validator {
...
func randValidator_() *Validator {
	val := NewValidator(randPubKey(), cmn.RandUint64(), "Reward", "Name")
...
func TestValidatorSetTotalVotingPowerOverflows(t *testing.T) {
	vset := NewValidatorSet([]*Validator{
		{Address: "a", VotingPower: math.MaxInt64, Accum: 0},
		{Address: "b", VotingPower: math.MaxInt64, Accum: 0},
		{Address: "c", VotingPower: math.MaxInt64, Accum: 0},
	})
...
func TestValidatorSetIncrementAccumOverflows(t *testing.T) {
	...
	vset := NewValidatorSet([]*Validator{
		...
		0: {Address: "a", VotingPower: math.MaxInt64, Accum: 0},
		// too big accum
		1: {Address: "b", VotingPower: 10, Accum: math.MaxInt64},
		// almost too big accum
		2: {Address: "c", VotingPower: 10, Accum: math.MaxInt64 - 5},
	})
...
func TestValidatorSetIncrementAccumUnderflows(t *testing.T) {
	// NewValidatorSet calls IncrementAccum(1)
	vset := NewValidatorSet([]*Validator{
		0: {Address: "a", VotingPower: math.MaxInt64, Accum: math.MinInt64},
		1: {Address: "b", VotingPower: 1, Accum: math.MinInt64},
	})
