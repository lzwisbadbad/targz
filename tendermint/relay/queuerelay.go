...
func (qr *QueueRelay) carry(lastResult bool) bool {
	...
	pktsProof, headers := qr.collectIBCPktsProof(lastResult)
	if len(pktsProof) == 0 {
		// 此时说明还没有发生跨链交易
	...
	if err := qr.sendIBCPackets(pktsProof, headers); err != nil {
		// 不能发送跨链交易到目标链
...
func (qr *QueueRelay) sendIBCPackets(pktsProofs []*PktsProof, headers []*Header_2_2) error {
	tx, err := qr.packTx(pktsProofs, headers)
	if err != nil {
		return err
	}
...
func (qr *QueueRelay) packTx(pktsProofs []*PktsProof, headers []*Header_2_2) (string, error) {
...
func (qr *QueueRelay) collectIBCPktsProof(lastResult bool) (pktsProofs []*PktsProof, headers []*Header_2_2) {
	if lastResult == false {
	...
	for {
		...
		pktsProof, header, ibcMsgCount := qr.getPacketsProof(height)
		if pktsProof == nil {
			return
		}
...
func (qr *QueueRelay) getPacketsProof(height int64) (*PktsProof, *Header_2_2, uint64) {
	blkResults, err := qr.getBlockResult(height)
	if err != nil {
		return nil, nil, 0
	}

	pktsProof, header, err := qr.getProof(height + 1)
	if err != nil {
	...
	return pktsProof, header, uint64(len(pktsProof.Packets))
}

func (qr *QueueRelay) getProof(headerHeight int64) (pktsProof *PktsProof, header *Header_2_2, err error) {
	var (
