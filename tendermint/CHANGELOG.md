...
- [rpc]: changed the output format for the `/status` endpoint (see https://godoc.org/github.com/bcbchain/tendermint/rpc/core#Status)
...
See github.com/bcbchain/tendermint/go-amino for details on the new format.
...
IMPROVEMENTS:
- rpc/client: can act as event bus subscriber (See https://github.com/bcbchain/tendermint/issues/945).
...
BUG FIXES:
 ...
 - consensus: fix wal autofile via https://github.com/bcbchain/tendermint/tmlibs/blob/master/CHANGELOG.md#032-october-2-2017
...
- Config and Flags:
  - The `config` map is replaced with a [`Config` struct](https://github.com/bcbchain/tendermint/blob/master/config/config.go#L11),
...
See our new [logging library](https://github.com/bcbchain/tendermint/tmlibs/log) and [blog post](https://tendermint.com/blog/abstracting-the-logger-interface-in-go) for more details
