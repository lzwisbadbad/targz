...
import (
	...
	tmpubsub "github.com/bcbchain/bclib/tendermint/tmlibs/pubsub"
)
...
func (c *HTTP) ABCIQueryEx(path string) (*ctypes.ResultABCIQueryEx, error) {
	return c.ABCIQueryExWithOptions(path)
}

func (c *HTTP) ABCIQueryExWithOptions(path string) (*ctypes.ResultABCIQueryEx, error) {
	result := new(ctypes.ResultABCIQueryEx)
	_, err := c.rpc.Call("abci_query_ex",
		map[string]interface{}{"path": path},
		result)
	if err != nil {
		return nil, errors.Wrap(err, "ABCIQueryEx")
	}
	return result, nil
}
