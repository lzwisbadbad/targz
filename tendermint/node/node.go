...
import (
	...
	"github.com/bcbchain/bclib/rpc/lib"
	"github.com/bcbchain/bclib/rpc/lib/server"

	bc "github.com/bcbchain/tendermint/blockchain"
	cfg "github.com/bcbchain/tendermint/config"
	cs "github.com/bcbchain/tendermint/consensus"
	"github.com/bcbchain/tendermint/evidence"
	mempl "github.com/bcbchain/tendermint/mempool"
	"github.com/bcbchain/tendermint/p2p"
	"github.com/bcbchain/tendermint/p2p/pex"
	"github.com/bcbchain/tendermint/p2p/trust"
	rpccore "github.com/bcbchain/tendermint/rpc/core"
	ctypes "github.com/bcbchain/tendermint/rpc/core/types"
	grpccore "github.com/bcbchain/tendermint/rpc/grpc"
	sm "github.com/bcbchain/tendermint/state"
	"github.com/bcbchain/tendermint/state/txindex"
	"github.com/bcbchain/tendermint/state/txindex/kv"
...
func DefaultNewNode(config *cfg.Config, logger log.Logger) (*Node, error) {
	switch len(config.ProxyApp) {
	case 1, 3:
		sc := sidechain.NewSideChain(nil)
		if sc.NeedSCGenesis() {
			err := sc.Genesis()
			if err != nil {
				panic(err)
		...
		if err := sc.CopyGenesisFiles(); err != nil {
			panic(err)
		}

		return NewNode(config,
			pvm.LoadOrGenFilePV(config.PrivValidatorFile()),
			proxy.DefaultClientCreator(config.ProxyApp[0], config.ABCI, config.DBDir()),
			DefaultGenesisDocProviderFunc(config),
			DefaultDBProvider,
			logger)
	default:
		return nil, errors.New("ProxyApp param is wrong,please check your ProxyApp，proxyApp.len:" + strconv.Itoa(len(config.ProxyApp)))
	}
}
...
func NewNode(config *cfg.Config,
	...
	// save block txs into mempool done cache
	height := blockStore.Height()
	var i int64
	if height > mempl.DoneCacheSize {
		i = height - mempl.DoneCacheSize + 1
	} else {
		i = 1
	}
	for ; i <= height; i++ {
		...
		mempl.GetDoneTxsCache().PushTxs(logger, i, block.Txs)
	}
	...
	relay.Init(config, logger, proxyApp)
	...
	if state.Validators.Size() == 1 {
		...
		if privValidator.GetAddress() == addr {
			fastSync = false
		}
	}
...
func (n *Node) OnStart() error {
	...
	aAddr := n.config.P2P.AAddress
	if strings.Contains(aAddr, "0.0.0.0") {
		aAddr = ""
	}
	// Create & add listener
	...
	l := p2p.NewDefaultListener(
		...
		address,
		aAddr,
		true, /*n.config.P2P.SkipUPNP*/ // 咱家根本木有UPNP，有了再说
...
func (n *Node) ConfigureRPC() {
	...
	rpccore.SetPrivatePeerIDs(n.config.P2P.PrivatePeerIDs)
}
...
func (n *Node) startRPC() ([]net.Listener, error) {
	...
	tlsEnable := true
	if n.config.RPC.KeyFile != "" && n.config.RPC.CertFile != "" {
		keyFile = filepath.Join(n.config.RootDir, "config", n.config.RPC.KeyFile)
		certFile = filepath.Join(n.config.RootDir, "config", n.config.RPC.CertFile)
		if sfi, err := os.Stat(keyFile); err != nil || !sfi.Mode().IsRegular() {
			tlsEnable = false
		}
		if sfi, err := os.Stat(certFile); err != nil || !sfi.Mode().IsRegular() {
			tlsEnable = false
		}
	}
	...
	for i, listenAddr := range listenAddrs {
		...
		var err error
		if tlsEnable {
			listener, err = rpcserver.StartHTTPAndTLSServer(listenAddr, mux, certFile, keyFile, rpcLogger)
		} else {
...
func (n *Node) makeNodeInfo(nodeID p2p.ID) p2p.NodeInfo {
	...
	nodeInfo.ListenAddr = p2pListener.String()
...
func loadGenesisDoc(db dbm.DB) (*types.GenesisDoc, error) {
	bytesLocal := db.Get(genesisDocKey)
	if len(bytesLocal) == 0 {
		return nil, errors.New("Genesis doc not found")
	}
	var genDoc *types.GenesisDoc
	err := cdc.UnmarshalJSON(bytesLocal, &genDoc)
	if err != nil {
		cmn.PanicCrisis(fmt.Sprintf("Failed to load genesis doc due to unmarshaling error: %v (bytes: %X)", err, bytesLocal))
	}
	return genDoc, nil
}
