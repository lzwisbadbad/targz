...
func AddNodeFlags(cmd *cobra.Command) {
	...
	// abci flags
	cmd.Flags().StringArray("proxy_app", config.ProxyApp, "Proxy app address array，one or three element(eg: '[tcp://address1:port1,tcp://address2:port2,tcp:address3:port3]'), or '[nilapp]'  for local testing.")
	cmd.Flags().String("abci", config.ABCI, "Specify abci transport (socket | grpc)")

	cmd.Flags().String("persist", "", "directory to use for a database")
...
func NewRunNodeCmd(nodeProvider nm.NodeProvider) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			logger.Info("Init tendermint", "version", version.Version)
