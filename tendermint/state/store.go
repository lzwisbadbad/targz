...
func LoadLastState(db dbm.DB) State {
	return loadState(db, lastStateKey)
}
...
func SaveLastState(db dbm.DB, lastState State) {
	saveState(db, lastState, lastStateKey)
}
...
// delLastQueueInfo del last queue height and last queue hash
func delLastQueueInfo(db dbm.DB, queueID string) {
	db.DeleteSync(calcLastQueueHeightKey(queueID))
	db.DeleteSync(calcLastQueueHashKey(queueID))
}
