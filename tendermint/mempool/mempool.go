...
import (
	...
	auto "github.com/bcbchain/bclib/tendermint/tmlibs/autofile"
	"github.com/bcbchain/bclib/tendermint/tmlibs/clist"
...
const memPoolWalMaxSize = 256 * 1024 * 1024

// Txs Done Cache
const DoneCacheSize = 128

var gDoneTxsCache *DoneTxsCache
var gDoneTxsCacheMtx sync.Mutex
...
func (mem *Mempool) GiTxSearch(tx string) (*abci.ResponseCheckTx, error) {
	...
	res, err := mem.giCache.Value(tx)
	if err != nil {
		mem.logger.Error("Error read GiCache", "err", err)
		return nil, err
	}
	if res.Data() == nil {
		res := new(abci.ResponseCheckTx)
		res.Code = 2018
		return res, nil
	}
...
func (mem *Mempool) GiTxCache(tx cmn.HexBytes, a interface{}) {
	if a == nil {
		if mem.giCache.Exists(tx.String()) {
			checkRes, errCheck := mem.GiTxSearch(tx.String())
			if errCheck == nil && checkRes.Code != abci.CodeTypeOK {
				mem.giCache.Add(tx.String(), time.Duration(mem.config.CTxCacheTime)*time.Second, nil)
			} else if errCheck != nil {
				mem.logger.Warn("GiTxSearch tx failed", "err", errCheck)
			}
			return
		}
	}
	mem.giCache.Add(tx.String(), time.Duration(mem.config.CTxCacheTime)*time.Second, a)
}

func (mem *Mempool) backupWal() {
	// write wal to wal.bak if size >= 256M
	size, err := mem.wal.Size()
	if err != nil {
		mem.logger.Error("Error get Wal file size", "err", err)
		return
	}
	if size >= memPoolWalMaxSize {
		walDir := mem.config.WalDir()
		if walDir == "" {
			cmn.PanicSanity(errors.Wrap(err, "Error get Mempool config file"))
		}

		if err = mem.wal.Close(); err != nil {
			mem.logger.Error("Error close Wal file ", "err", err)
			return
		}

		if err = os.Rename(walDir+"/wal", walDir+"/wal.bak"); err != nil {
			mem.logger.Error("Error rename Wal file ", "err", err)
			return
		}
...
func (mem *Mempool) CheckTx(tx types.Tx, cb func(*abci.Response)) (err error) {
	...
	// doneTxsCache
	if GetDoneTxsCache().Exists(mem.logger, tx) {
		return ErrTxInCache
	}
...
func (mem *Mempool) resCbNormal(req *abci.Request, res *abci.Response) {
	...
	case *abci.Response_CheckTx:
		...
		if (r.CheckTx.Code == abci.CodeTypeOK) && (!GetDoneTxsCache().Exists(mem.logger, tx)) {
			mem.counter++
			memTx := &mempoolTx{
				counter: mem.counter,
				height:  mem.height,
...
func GetDoneTxsCache() *DoneTxsCache {
	gDoneTxsCacheMtx.Lock()
	defer gDoneTxsCacheMtx.Unlock()

	if gDoneTxsCache == nil {
		gDoneTxsCache = &DoneTxsCache{
			mtx:     &gDoneTxsCacheMtx,
			maxSize: DoneCacheSize,
			map_:    make(map[string]int),
			list:    list.New(),
		}
	}

	return gDoneTxsCache
}

// Reset resets the txCache to empty.
func (cache *DoneTxsCache) Reset() {
	cache.mtx.Lock()
	cache.map_ = make(map[string]int)
	cache.list.Init()
	cache.mtx.Unlock()
}
...
func (cache *DoneTxsCache) Exists(logger log.Logger, tx types.Tx) bool {
	...
	_, exists := cache.map_[string(tx)]
	if exists {
		logger.Info("Tx already in done cache")
	}
	cache.mtx.Unlock()
	return exists
}
...
func (cache *DoneTxsCache) PushTxs(logger log.Logger, height int64, txs types.Txs) {
	...
	// return if txs is nil
	if len(txs) <= 0 {
		return
	}

	// manage list
	//logger.Info("PushTxs", "list len", cache.list.Len())
	if cache.list.Len() >= cache.maxSize {
		popped := cache.list.Front()
		doneTxs := popped.Value.(DoneTxs)
		// delete  cache.map_
		for _, tx := range doneTxs.txs {
			i, _ := cache.map_[string(tx)]
			if i > 0 {
				cache.map_[string(tx)] = i - 1
			} else {
				delete(cache.map_, string(tx))
			}
		}
	...
	// insert new data and map
	doneTxs := DoneTxs{
		height: height,
		txs:    txs,
	}
	for _, tx := range txs {
		i, exists := cache.map_[string(tx)]
		if exists {
			cache.map_[string(tx)] = i + 1
		} else {
			cache.map_[string(tx)] = 0
		}

	}
	cache.list.PushBack(doneTxs)
	return
}
