...
``$GOPATH/bin`` in your
``$PATH``; see `here <https://github.com/bcbchain/tendermint/wiki/Setting-GOPATH>`__ for more info.

Then run

::

    go get -u github.com/bcbchain/tendermint/abci/cmd/abci-cli
...
::
    ...
    cd $GOPATH/src/github.com/bcbchain/tendermint/abci
    make get_tools
...
tuned for a future release that `makes this output more human-readable <https://github.com/bcbchain/tendermint/abci/issues/32>`__.
...
You'll also need to fetch the relevant repository, from `here <https://github.com/bcbchain/tendermint/js-abci>`__ then install it. As go devs, we
keep all our code under the ``$GOPATH``, so run:

::

    go get github.com/bcbchain/tendermint/js-abci &> /dev/null
    cd $GOPATH/src/github.com/bcbchain/tendermint/js-abci/example
    npm install
    cd ..
