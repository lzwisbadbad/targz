...
-  To *prevote* or *precommit* something means to broadcast a `prevote
   vote <https://godoc.org/github.com/bcbchain/tendermint/types#Vote>`__
   or `first precommit
   vote <https://godoc.org/github.com/bcbchain/tendermint/types#FirstPrecommit>`__
   for something.
...
-  Nodes gossip to nodes lagging in blockchain height with block
   `commits <https://godoc.org/github.com/bcbchain/tendermint/types#Commit>`__
   for older blocks.
...
voting power. (see
`implementation <https://github.com/bcbchain/tendermint/blob/develop/types/validator_set.go>`__)
