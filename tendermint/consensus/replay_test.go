...
func startNewConsensusStateAndWaitForBlock(t *testing.T, lastBlockHeight int64, blockDB dbm.DB, stateDB dbm.DB) {
	...
	state, _ := sm.LoadStateFromDBOrGenesisFile(stateDB, consensusReplayConfig)
...
LOOP:
	for {
		...
		state, _ := sm.MakeGenesisStateFromFile(consensusReplayConfig)
		privValidator := loadPrivValidator(consensusReplayConfig)
...
func testHandshakeReplay(t *testing.T, nBlocks int, mode uint) {
	...
	appState := abci.ByteToAppState(res.LastAppState)
	if !bytes.Equal(latestAppHash, appState.AppHash) {
		t.Fatalf("Expected app hashes to match after handshake/replay. got %X, expected %X", appState.AppHash, latestAppHash)
	}
...
func buildAppStateFromChain(proxyApp proxy.AppConns, stateDB dbm.DB,
	...
	// TODO: get the genesis bytes (https://github.com/bcbchain/tendermint/issues/1224)
...
func stateAndStore(config *cfg.Config, pubKey crypto.PubKey) (dbm.DB, sm.State, *mockBlockStore) {
	stateDB := dbm.NewMemDB()
	state, _ := sm.MakeGenesisStateFromFile(config)
