...
#like ["tcp://127.0.0.1:46658"] or ["tcp://127.0.0.1:46658","tcp://127.0.0.1:46659","tcp://127.0.0.1:46660"]
proxy_app = [{{range $i,$e := .BaseConfig.ProxyApp }}{{if $i}},{{end}}"{{$e}}"{{end}}]
...
# Output directory for logging, log file may be very large
...
# Path to the Validators file containing all the genesis validators
validators_file = "{{ .BaseConfig.Validators }}"
...
cert_file = "{{ .RPC.CertFile }}"
key_file = "{{ .RPC.KeyFile }}"
...
# Address to ANNOUNCE i am listening for incoming connections
aaddr = "{{ .P2P.AAddress }}"
